...
def validate(P):
    if len(P) == 0: 
        return True
    if ((len(P) <= 2) and (P.isdigit()) and (int(P) > 0)):
        return True
    else:
        return False
...
def steps_main(step: int):
    global steps_window,steps_levels, zona_label, dce_check, catT2,catDWI, eep, info, mapa_flag, psa_value, fpsa_value, psa_date1, psa_date2, psa_date3, t1,t2,t3,t4,t5,t6,hemo,neuro,vesi,huesos,organos,linfa,uretra, auxframe2, zona, lesion_flag
    mapa_flag = False
    ...
    if step == 1:
        ...
        Label(steps_title, text="Sobre el paciente...",bg="#2CC",font=("Roboto",12),fg="#000").pack(fill=X,ipady=5,ipadx=20)
        steps_window = Frame(steps_levels,background="#444")
        steps_window.pack(side=LEFT)
        ...
        psa_value = Entry(auxframe1,width=4,font=("Roboto",12),fg="#FFF",bd=0,bg="#666",insertbackground="#2CC", validate="key", validatecommand=(root.register(validate), '%P'))
        psa_value.pack(side=LEFT)
        Label(auxframe1, text="f PSA:",bg="#555",font=("Roboto",10),fg="#FFF",width=7).pack(side=LEFT)
        fpsa_value = Entry(auxframe1,width=4,font=("Roboto",12),fg="#FFF",bd=0,bg="#666",insertbackground="#2CC", validate="key", validatecommand=(root.register(validate), '%P'))
        fpsa_value.pack(side=LEFT)
        Label(auxframe1, text="Fecha(DD/MM/AA):",bg="#555",font=("Roboto",10),fg="#FFF",width=15).pack(side=LEFT)
        psa_date1= Entry(auxframe1,width=2,font=("Roboto",12),fg="#FFF",bd=0,bg="#666",insertbackground="#2CC", validate="key", validatecommand=(root.register(validate), '%P'))
        psa_date1.pack(side=LEFT,padx=(0,2))
        Label(auxframe1, text="/",bg="#555",font=("Roboto",10),fg="#FFF",width=1).pack(side=LEFT)
        psa_date2= Entry(auxframe1,width=2,font=("Roboto",12),fg="#FFF",bd=0,bg="#666",insertbackground="#2CC", validate="key", validatecommand=(root.register(validate), '%P'))
        psa_date2.pack(side=LEFT,padx=(0,2))
        Label(auxframe1, text="/",bg="#555",font=("Roboto",10),fg="#FFF",width=1).pack(side=LEFT)
        psa_date3= Entry(auxframe1,width=4,font=("Roboto",12),fg="#FFF",bd=0,bg="#666",insertbackground="#2CC", validate="key", validatecommand=(root.register(validate), '%P'))
        ...
        separator.pack(side=LEFT,ipadx=2)
        steps_window2 = Frame(steps_levels,background="#444")
        steps_window2.pack(side=RIGHT,fill=Y)
        Label(steps_window2, text="Observaciones Realizadas",bg="#444",font=("Roboto",11),fg="#FFF").pack(ipady=2,ipadx=20,anchor=W,pady=(100,0))
        ...
        Label(steps_window2, text="Conclusiones",bg="#444",font=("Roboto",12),fg="#FFF").pack(ipady=1,ipadx=20,anchor=W,pady=(40,0)) 
    ...
    for file in allfiles:
        ...
        if (ext == ".IMA") or (ext == ".dcm"):
            ...
            if  temp_uid not in sec_uids:
                try: 
                    if temp_dcm.ContrastBolusAgent:
                        dce_secs.append(secuencia("".join([temp_dcm.SequenceName,"-> TE: ",str(temp_dcm.EchoTime),", TR: "+str(temp_dcm.RepetitionTime),", ",str(temp_dcm.ScanOptions),", UID: ",temp_uid[-4:-1]])))
                except:
                    secuencias.append(secuencia("".join([temp_dcm.SequenceName,"-> TE: ",str(temp_dcm.EchoTime),", TR: ",str(temp_dcm.RepetitionTime),", ",str(temp_dcm.ScanOptions),", UID: ",temp_uid[-4:-1]])))
...
def generator (save_directory: str):
    ...
    doc.append("\n")
    rp_name = str(secuencias[0].dcm_serie[0].PatientID)+"_Reporte_PI-RADS"
