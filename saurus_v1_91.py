...
def refresh_report():
    ...
    for n, obs in enumerate(observaciones):
        ...
        Button(mini_report, text="Edit", font=("Roboto",12,"bold"), bg="#AA5", cursor="hand2", bd=0, command=lambda step=6:steps_main(step)).pack(side=LEFT,ipadx=1,fill=Y)
        temp_bg = "#555"
...
def steps_main(step: int):
    ...
    if step == 1:
        ...
        '''
        Deberia abrir la misma ventana q para la creacion de observaciones (menos las dimensiones, si esta mal medido hacela de nuevo macho)
        Deberia cargar los datos que ya tenia como default en los casilleros de la nueva ventana?
        Como minimo abrir de nuevo la ventana y guardar los datos NO en una nueva observación (como dice el paso step del boton VER)
        Ver que se mantenga el ID
        Tratar de evitar usar flags y crear todo de nuevo pero que llame a los steps bien.
        '''
        pass
