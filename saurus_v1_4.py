...
def focus_cv(event, arg: Canvas):
    ...
    for sec in secuencias:
        if sec.incv == cv: 
            focused_sec = sec
            break
    if info_cv.get(): info_cv_gen(focused_sec)
def unfocus_cv(event):
    for cvs in cv_master:
        cvs.delete("focus_check","cv_info")
...
def slice_selector(event):
    if event.delta > 0 and focused_sec.slice < focused_sec.depth-1: focused_sec.slice += 1
    elif event.delta < 0 and focused_sec.slice > 0: focused_sec.slice -= 1
    refresh_canvas(focused_sec) 
...
def bnc(event,tipo: str):
    sec = focused_sec
    if tipo == "c+" and sec.alpha < 0.5:        sec.adjust_img_serie(0.005,0)
    elif tipo == "c-" and sec.alpha >= 0.01:    sec.adjust_img_serie(-0.005,0)
    elif tipo == "b+" and sec.beta < 100:       sec.adjust_img_serie(0,5)
    elif tipo == "b-" and sec.beta >= 5:        sec.adjust_img_serie(0,-5)
    refresh_canvas(sec) 
